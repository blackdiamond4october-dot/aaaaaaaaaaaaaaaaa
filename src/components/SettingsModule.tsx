...
export const SettingsModule: React.FC = () => {
  ...
  const handleExportData = () => {
    ...
    a.href = url;
    a.download = `zenpos_backup_${new Date().toISOString().split('T')[0]}.json`;
    a.click();
  };
        ...
        <div className="max-w-3xl mx-auto space-y-12">
            ...
            <div className="space-y-10 animate-in fade-in slide-in-from-bottom-2 duration-400">
              ...
              <div className="space-y-6">
                <div className="space-y-4">
                  <label className="text-[10px] font-black text-app-muted uppercase tracking-[0.2em] block">Admin Account Setup</label>
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4 bg-app-bg border border-app-border rounded-xl p-4">
                    <div className="space-y-2">
                       <label className="text-[9px] font-black text-app-muted uppercase tracking-widest block">Admin Name</label>
                       <input 
                         value={settings.adminName || ''} 
                         onChange={e => setSettings({...settings, adminName: e.target.value})} 
                         className="w-full bg-app-surface border border-app-border rounded px-4 py-2.5 outline-none text-[11px] font-black uppercase text-app-text focus:border-[#ef444455] transition-all" 
                         placeholder="Sarah J." 
                       />
                    </div>
                    <div className="space-y-2">
                       <label className="text-[9px] font-black text-app-muted uppercase tracking-widest block">Admin Setup Password</label>
                       <input 
                         type="password"
          ...
          {activeTab === 'Database' && (
            <div className="space-y-10 animate-in fade-in slide-in-from-bottom-2 duration-400">
              <div className="flex items-center justify-between border-b border-app-border pb-6">
                <div>
                  <h3 className="text-xl font-black tracking-tighter text-[#14b8a6] flex items-center gap-3 uppercase">
                    <Database size={24} /> 
                    SYSTEM DATABASE
                  </h3>
                  <p className="text-[10px] font-bold text-app-muted uppercase tracking-widest mt-1 italic">Backup and restore management</p>
                </div>
              </div>

              <div className="space-y-8">
                <div className="bg-app-bg border border-app-border rounded-xl p-6 flex items-center justify-between">
                  <div>
                    <h4 className="text-sm font-black text-app-text uppercase tracking-tight">Export Database Backup</h4>
                    <p className="text-[10px] text-app-muted uppercase font-bold tracking-widest mt-1">Download a full JSON backup of all system data.</p>
                  </div>
                  <button onClick={handleExportData} className="bg-[#6366f1] text-white px-6 py-2.5 rounded font-black text-[10px] uppercase tracking-widest hover:brightness-110 shadow-lg shadow-[#6366f133] min-w-[140px]">
                    Export JSON
                  </button>
                </div>

                <div className="bg-[#ef444408] border border-[#ef444433] rounded-xl p-6 flex items-center justify-between">
                  <div>
                    <h4 className="text-sm font-black text-[#ef4444] uppercase tracking-tight">Restore Database</h4>
                    <p className="text-[10px] text-app-muted uppercase font-bold tracking-widest mt-1">Warning: This will overwrite current data.</p>
                  </div>
                  <div className="min-w-[140px]">
                    ...
                    />
                    <label htmlFor="db-backup-upload" className="bg-[#ef4444] cursor-pointer text-white px-6 py-3 rounded font-black text-[10px] uppercase tracking-widest flex items-center justify-center hover:brightness-110 shadow-lg shadow-[#ef444433]">
                      Restore JSON
                    </label>
                  </div>
                </div>
            ...
            <div className="space-y-8 animate-in fade-in slide-in-from-bottom-2 duration-400">
              <div className="flex items-center justify-between border-b border-app-border pb-6">
                <div>
                  ...
                  <p className="text-[10px] font-bold text-app-muted uppercase tracking-widest mt-1 italic">Hardware Output Configuration</p>
                </div>
              </div>
              <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                <div className="space-y-2">
                  <label className="text-[10px] font-black text-app-muted uppercase tracking-[0.2em] block">Receipt Size Format</label>
                  <select 
                    value={settings.receiptSize || '3-inch'} 
                    onChange={e => setSettings({...settings, receiptSize: e.target.value as any})} 
                    className="w-full bg-app-bg border border-app-border rounded px-4 py-3 outline-none text-xs font-black uppercase tracking-widest text-[#6366f1] focus:border-[#6366f155] transition-all appearance-none"
                  >
                    <option value="2-inch">58mm (2-inch)</option>
                    <option value="3-inch">80mm (3-inch)</option>
                    <option value="A5">A5 Half</option>
                    <option value="A4">A4 Full</option>
                  </select>
                </div>
                <div className="space-y-2">
                  <label className="text-[10px] font-black text-app-muted uppercase tracking-[0.2em] block">Receipt Footer Notes</label>
                  <input 
                    value={settings.receiptFooter || 'Thank you for your business!'} 
                    onChange={e => setSettings({...settings, receiptFooter: e.target.value})} 
                    className="w-full bg-app-bg border border-app-border rounded px-4 py-3 outline-none font-mono text-[11px] text-app-text" 
                    placeholder="Footer message..."
                  />
                </div>
                <div className="md:col-span-2">
                  <label className="flex items-center justify-between p-5 bg-app-bg border border-app-border rounded-lg cursor-pointer hover:border-[#6366f155] transition-all group overflow-hidden relative">
                    <div className="relative z-10 space-y-1">
                      <span className="text-[10px] font-black uppercase tracking-wider text-app-muted group-hover:text-app-text block">Render Logo on Hardcopy</span>
                      <span className="text-[9px] font-bold uppercase tracking-wider text-app-muted/50 block">Include shop raster asset on top of receipt</span>
                    </div>
                    <div className="relative z-10">
                      <input 
                        type="checkbox" 
                    ...
                    <div className="absolute top-0 right-0 w-16 h-full bg-[#6366f105] opacity-0 group-hover:opacity-100 transition-opacity" />
                  </label>
                </div>
              </div>
            </div>
          ...
          {activeTab === 'Theme' && (
            <div className="space-y-8 animate-in fade-in slide-in-from-bottom-2 duration-400">
              <div className="flex items-center justify-between border-b border-app-border pb-6">
                <div>
                  <h3 className="text-xl font-black tracking-tighter text-[#f59e0b] flex items-center gap-3 uppercase">
                    <Palette size={24} /> 
                    AESTHETICS.SYS
                  </h3>
                  <p className="text-[10px] font-bold text-app-muted uppercase tracking-widest mt-1 italic">Visual Appearance Parameters</p>
                </div>
              </div>
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                {[
                  { id: 'dark', name: 'Void Deep', accent: 'bg-app-header' },
                  { id: 'light', name: 'Opal Glare', accent: 'bg-[#f4f4f5] border border-zinc-200' },
                  { id: 'system', name: 'OS Sync', accent: 'bg-gradient-to-br from-[#18181b] to-[#f4f4f5]' }
                ].map(themeItem => (
                  <button 
                    key={themeItem.id}
                    onClick={() => setSettings({...settings, theme: themeItem.id})}
                    className={cn(
                      "flex items-center justify-between p-5 bg-app-bg rounded-lg border transition-all text-left group",
                      settings.theme === themeItem.id || (!settings.theme && themeItem.id === 'dark')
                        ? "border-[#f59e0b] shadow-[0_0_15px_rgba(245,158,11,0.1)]" 
                        : "border-app-border hover:border-[#f59e0b55]"
                    )}
                  >
                    <div>
                      <p className={cn(
                        "text-[11px] font-black uppercase tracking-widest",
                        settings.theme === themeItem.id ? "text-[#f59e0b]" : "text-app-text"
                      )}>{themeItem.name}</p>
                      <p className="text-[9px] text-app-muted italic font-bold tracking-wider mt-1 uppercase">UI Layout Profile</p>
                    </div>
                    <div className={cn("w-6 h-6 rounded-full flex-shrink-0", themeItem.accent)}></div>
                  </button>
                ))}
              </div>
            </div>
